...
$tagIdArr = TorrentTag::query()->where('torrent_id', $id)->get()->pluck('tag_id')->toArray();
...
if (! isset($CURUSER) || ($CURUSER['id'] != $row['owner'] && ! user_can('torrentmanage'))) {
    echo '<h1 align="center">'.$lang_edit['text_cannot_edit_torrent'].'</h1>';
    ...
    echo "<tr><td class='colhead' colspan='2' align='center'>".htmlspecialchars($row['name']).'</td></tr>';
    ...
    echo '<tr><td class="rowhead">'.$lang_edit['row_description'].'<font color="red">*</font></td><td class="rowfollow">';
    ...
    */
    ...
    echo '<tr><td class="toolbox" colspan="2" align="center"><input id="qr" type="submit" value="'.$lang_edit['submit_edit_it'].'" /> <input type="reset" value="'.$lang_edit['submit_revert_changes']."\" /></td></tr>\n";
    echo "</table>\n";
    echo "</form>\n";
    if (user_can('torrent-delete') && user_can('torrentmanage')) {
        ...
        echo "<tr><td class=\"colhead\" align=\"left\" style='padding-bottom: 3px' colspan=\"2\">".$lang_edit['text_delete_torrent'].'</td></tr>';
        ...
        echo "<tr><td class=\"toolbox\" colspan=\"2\" align=\"center\"><input type=\"submit\" style='height: 25px' value=\"".$lang_edit['submit_delete_it']."\" /></td></tr>\n";
        echo '</table>';
        echo "</form>\n";
    }
    $json_sticky_series = json_encode([4, 6, 12, 24, 36, 48, 72, 168, 360]);
    echo <<<EOT
<script>
jQuery(function($){
...
Nexus::js('js/ptgen.js', 'footer', true);
$customFieldJs = <<<JS
...
JS;
Nexus::js($customFieldJs, 'footer', false);
stdfoot();
