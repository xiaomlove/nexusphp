...
GROUP BY u.id
	...
	$count = $ipCountRows ? (int) array_values((array) $ipCountRows[0])[0] : 0;
	...
	[$pagertop, $pagerbottom, $limit, $offsetStart, $rowsPerPage] = pager($perpage, $count, "{$_SERVER['PHP_SELF']}?ip=$ip&mask=$mask&order=$order&");
...
$limit";
	...
	$userIpRows = \Nexus\Database\NexusDB::select($query);
...
"<td class=colhead align=center>".$lang_ipsearch['col_invited_by']."</td>");
	...
	foreach ($userIpRows as $user)
	{
		...
		$iphistory = (int) \Nexus\Database\NexusDB::table('iplog')->where('userid', (int) $user['id'])->distinct()->count('ip');
