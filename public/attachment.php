...
if ($Attach->enable_attachment()) {
            ...
            if ($storageDriver == 'local' || ! $isimage) {
                ...
                if ($isimage) { // the uploaded file is a image
                    ...
                    if ($imagesize) {
                        ...
                        $warning = $lang_attachment['text_invalid_image_file'];
                    }
                }
                if (! $abandonorig) {
    ...
    echo '<form enctype="multipart/form-data" name="attachment" method="post" action="attachment.php?callback_func='.($_REQUEST['callback_func'] ?? '').'">';
    echo '<tr>';
    ...
    if ($warning) {
        ...
        echo '<b>'.$lang_attachment['text_left'].'</b><font color="red">'.$count_left.'</font>'.$lang_attachment['text_of'].$count_limit.'&nbsp;&nbsp;&nbsp;<b>'.$lang_attachment['text_size_limit'].'</b>'.mksize($size_limit).'&nbsp;&nbsp;&nbsp;<b>'.$lang_attachment['text_file_extensions'].'</b>';
        $allowedextsblock = '';
