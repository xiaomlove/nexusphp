...
loggedinorreturn();
$userInfo = User::query()->findOrFail($CURUSER['id']);
        ...
        switch ($action) {
            case 'personal':
                if ($type == 'save') {
                    ...
                    if ($showschool = 'yes') {
                        $school = $_POST['school'];
                        $updateset['school'] = (string) $school;
                    }
                    ...
                    $updateset['parked'] = (string) $parked;
                    $updateset['acceptpms'] = (string) $acceptpms;
                    $updateset['deletepms'] = (string) $deletepms;
                    $updateset['savepms'] = (string) $savepms;
                    $updateset['commentpm'] = (string) $commentpm;
                    $updateset['gender'] = (string) $gender;
                    if (is_valid_id($country)) {
                        $updateset['country'] = (int) $country;
                    }
                    if (is_valid_id($download)) {
                        $updateset['download'] = (int) $download;
                    }
                    if (is_valid_id($upload)) {
                        $updateset['upload'] = (int) $upload;
                    }
                    if (is_valid_id($isp)) {
                        $updateset['isp'] = (int) $isp;
                    }
                    //	$updateset['tzoffset'] = (string) $tzoffset;

                    $updateset['info'] = (string) $info;
                    $updateset['tracker_url_id'] = (int) ($_POST['tracker_url_id'] ?? 0);
                ...
                foreach (NexusDB::select('SELECT id, name FROM countries ORDER BY name') as $ct_a) {
                ...
                foreach (NexusDB::select('SELECT id, name FROM isp ORDER BY id ASC') as $isp_a) {
                ...
                foreach (NexusDB::select('SELECT id, name FROM downloadspeed ORDER BY id') as $ds_b) {
                ...
                foreach (NexusDB::select('SELECT id, name FROM uploadspeed ORDER BY id') as $us_b) {
                ...
                $options = '';
                $text = '';
                foreach (NexusDB::select("SELECT * FROM bitbucket WHERE public = '1'") as $sor) {
                ...
                tr_small($lang_usercp['row_account_parked'],
                    '<input type=checkbox name=parked'.($CURUSER['parked'] == 'yes' ? ' checked' : '').' value=yes>'.$lang_usercp['checkbox_pack_my_account'].'<br /><font class=small size=1>'.$lang_usercp['text_account_pack_note'].'</font>', 1);
...
<input type=radio name=gender'.($CURUSER['gender'] == 'Male' ? ' checked' : '').' value=Male>'.$lang_usercp['radio_male'].'<input type=radio name=gender'.($CURUSER['gender'] == 'Female' ? ' checked' : '').' value=Female>'.$lang_usercp['radio_female'], 1);
                ...
                if ($showschool == 'yes') {
                    ...
                    foreach (NexusDB::select('SELECT id, name FROM schools ORDER BY name') as $sc_a) {
  ...
  "\"><br />\n".$lang_usercp['text_avatar_note'].($enablebitbucket_main == 'yes' ? $lang_usercp['text_bitbucket_note'] : ''), 1);
            ...
            case 'tracker':
                ...
                if ($type == 'save') {
                    ...
                    {
                        global $_POST;
                        foreach (NexusDB::select('SELECT id FROM '.$dbtable) as $a) {
                    ...
                    $updateset['hidehb'] = (string) $hidehb;
                    if ($showextinfo['imdb'] == 'yes') {
                        ...
                        $updateset['showimdb'] = (string) $showimdb;
                    }
                    ...
                    $updateset['showdescription'] = (string) $showdescription;
                    if ($enablenfo_main == 'yes') {
                        ...
                        $updateset['shownfo'] = (string) $shownfo;
                    }
                    ...
                    $updateset['showsmalldescr'] = (string) $showsmalldescr;
                    if ($_POST['showcomment'] == 'yes') {
                        $showcomment = 'yes';
                    } else {
                        $showcomment = 'no';
                    }
                    $updateset['showcomment'] = (string) $showcomment;
                ...
                */
                ...
                $ss_sa = [];
                foreach (NexusDB::select('SELECT * FROM stylesheets') as $ss_a) {
                    $ss_id = $ss_a['id'];
                    $ss_name = $ss_a['name'];
                    $ss_sa[$ss_name] = $ss_id;
                }
		...
		<b>'.$lang_usercp['text_comments_reviews'].': </b><br /><input type=checkbox name=showcomnum '.($CURUSER['showcomnum'] == 'yes' ? ' checked' : '').' value=yes>'.$lang_usercp['text_show_comment_number'].($showtooltipsetting ? '<select name="showlastcom" style="width: 70px;"><option value="yes" '.($CURUSER['showlastcom'] != 'no' ? ' selected' : '').'>'.$lang_usercp['select_with'].'</option><option value="no" '.($CURUSER['showlastcom'] == 'no' ? ' selected' : '').'>'.$lang_usercp['select_without'].'</option></select>'.$lang_usercp['text_last_comment_on_tooltip'] : ''), 1);
            ...
            case 'forum':
                ...
                if ($type == 'save') {
                    ...
                    $updateset['avatars'] = (string) $avatars;
                    if ($showtooltipsetting) {
                        $updateset['showlastpost'] = (string) $ttlastpost;
                    }
                    $updateset['signatures'] = (string) $signatures;
                    $clicktopic = $_POST['clicktopic'];
                    $updateset['clicktopic'] = (string) $clicktopic;
            ...
            case 'security':
                if ($type == 'confirm') {
                    ...
                    // 验证旧密码
                    $challenge = NexusDB::cache_get(get_challenge_key($userInfo->username));
                    if (empty($challenge)) {
                    ...
                    if (! empty($twoStepSecretHash)) {
                        ...
                        if (empty($CURUSER['two_step_secret'])) {
                            ...
                            $updateset['two_step_secret'] = (string) $twoStepSecret;
                        } else {
                            // unbind
                    ...
                    if ($chpassword != '') {
                        ...
                        $updateset['secret'] = (string) $sec;
                        $updateset['passhash'] = (string) $passhash;
                        $authKey = mksecret();
                        $updateset['auth_key'] = (string) $authKey;
                    ...
                    if ($resetpasskey == 1) {
                        ...
                        $updateset['passkey'] = (string) $passkey;
                    }
                    if ($changedemail == 1) {
                        $sec = mksecret();
                        $hash = md5($sec.$email.$sec);
                        $obemail = rawurlencode($email);
                        $updateset['editsecret'] = (string) $sec;
...
EOD;
                    ...
                    $user = $CURUSER['id'];
                    NexusDB::transaction(function () use ($user, $updateset) {
                        NexusDB::table('users')
                            ->where('id', (int) $user)
                            ->update($updateset);
                    ...
                    NexusDB::cache_del(get_challenge_key($userInfo->username));
                    header("Location: $to");
                }
...
usercpmenu();
// Comment Results
$commentcount = NexusDB::table('comments')->where('user', (int) $CURUSER['id'])->count();
...
if (! $forumposts = $Cache->get_value('user_'.$CURUSER['id'].'_post_count')) {
    $forumposts = NexusDB::table('posts')->where('userid', (int) $CURUSER['id'])->count();
...
if ($enablelocation_tweak == 'yes') {
    [$loc_pub, $loc_mod] = get_ip_location($CURUSER['ip']);
...
if ($prolinkpoint_bonus) {
    $prolinkclick = NexusDB::table('prolinkclicks')->where('userid', (int) $CURUSER['id'])->count();
...
if (get_setting('seed_box.enabled') == 'yes') {
    ...
    $res = SeedBoxRecord::query()->where('uid', $CURUSER['id'])->where('type', SeedBoxRecord::TYPE_USER)->get();
    if ($res->count() > 0) {
...
'</tr>';
$readpostsTopics = NexusDB::select(
