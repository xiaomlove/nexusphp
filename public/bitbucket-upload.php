...
//	$ret=($it==1)?imagegif($thumb, $tgtfile): ($it==2)?imagejpeg($thumb, $tgtfile):imagepng($thumb, $tgtfile);
	...
	$public = (!isset($_POST['public']) || $_POST['public'] != 'yes') ? '0' : '1';
	\Nexus\Database\NexusDB::insert('bitbucket', [
		'owner' => (int) $CURUSER['id'],
		'name' => (string) $filename,
		'added' => date("Y-m-d H:i:s"),
		'public' => (string) $public,
	]);
	\Nexus\Database\NexusDB::table('users')->where('id', (int) $CURUSER['id'])->update(['avatar' => (string) $url]);
