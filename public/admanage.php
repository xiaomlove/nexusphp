...
if ($action == 'del') {
    ...
    NexusDB::table('advertisements')->where('id', (int) $id)->delete();
    ...
    if (! $id) {
        ...
        $rowObj = NexusDB::table('advertisements')->where('id', (int) $id)->first();
        if (! $rowObj) {
            stderr($lang_admanage['std_error'], $lang_admanage['std_invalid_id']);
        } else {
            $row = (array) $rowObj;
            ...
            echo '<form method="post" action="?action=submit&amp;position='.$position.'">';
    ...
    $validpos = ['header', 'footer', 'belownav', 'belowsearchbox', 'torrentdetail', 'comment', 'interoverforums', 'forumpost', 'popup'];
    if (! in_array($position, $validpos)) {
        ...
        echo '<form method="post" action="?action=submit&amp;position='.htmlspecialchars($position).'">';
        echo '<input type="hidden" name="isedit" value="0" />';
    ...
    if (! in_array($position, $validpos)) {
        ...
        if ($_POST['isedit']) {
            ...
            $lastId = NexusDB::table('advertisements')->orderByDesc('id')->value('id');
            $adid = $lastId ? ((int) $lastId) + 1 : 1;
        }
        $name = $_POST['ad']['name'];
        ...
        if (get_user_class() >= $allowxhtmlclass) {
            $validtype = ['bbcodes', 'xhtml', 'text', 'image', 'flash'];
        } else {
            $validtype = ['bbcodes', 'text', 'image', 'flash'];
        }
...
<?php
        $perpage = 20;
    $num = (int) NexusDB::table('advertisements')->count();
    if (! $num) {
        echo '<p align="center">'.$lang_admanage['text_no_ads_yet'].'</p>';
    } else {
        [$pagertop, $pagerbottom, $limit, $offsetStart, $rowsPerPage] = pager($perpage, $num, '?');
        $adRows = NexusDB::table('advertisements')
            ->orderByDesc('id')
...
foreach ($adRows as $row) {
    $row = (array) $row;
    $clickcount = (int) NexusDB::table('adclicks')->where('adid', (int) $row['id'])->count();
    ?>
<tr>
