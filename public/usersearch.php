...
if (count($_GET) > 0 && ! $_GET['h']) {
    ...
    if ($names[0] !== '') {
        ...
        if (is_array($names_inc)) {
            ...
            foreach ($names_inc as $name) {
                if (! haswildcard($name)) {
                    $name_is .= (isset($name_is) ? ' OR ' : '').'u.username = '.DB::getPdo()->quote($name);
                } else {
                    $name = str_replace(['?', '*'], ['_', '%'], $name);
                    $name_is .= (isset($name_is) ? ' OR ' : '').'u.username LIKE '.DB::getPdo()->quote($name);
                }
            }
            $where_is .= $name_is.')';
            unset($name_is);
    ...
    if ($emaila[0] !== '') {
        ...
        foreach ($emaila as $email) {
            if (strpos($email, '*') === false && strpos($email, '?') === false
                ...
                $email_is .= (isset($email_is) ? ' OR ' : '').'u.email = '.DB::getPdo()->quote($email);
            } else {
                $sql_email = str_replace(['?', '*'], ['_', '%'], $email);
                $email_is .= (isset($email_is) ? ' OR ' : '').'u.email LIKE '.DB::getPdo()->quote($sql_email);
            }
        }
        $where_is .= $email_is.')';
        ...
        if (is_array($comments_inc)) {
            ...
            foreach ($comments_inc as $comment) {
                if (! haswildcard($comment)) {
                    $comment_is .= (isset($comment_is) ? ' OR ' : '').'u.modcomment LIKE '.DB::getPdo()->quote('%'.$comment.'%');
                } else {
    ...
    $countRow = NexusDB::select($queryc);
    $count = (int) (array_values((array) ($countRow[0] ?? []))[0] ?? 0);

    $q = isset($q) ? ($q.'&') : '';

    $perpage = 30;

    [$pagertop, $pagerbottom, $limit] = pager($perpage, $count, $_SERVER['REQUEST_URI'].'?'.$q);

    $query .= $limit;

    $userRows = NexusDB::select($query);
        ...
        foreach ($userRows as $user) {
            ...
            if ($user['ip']) {
                ...
                if (filter_var($user['ip'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
                    $nip = ip2long($user['ip']);
                    $banCount = (int) (NexusDB::table('bans')
                        ->where('first', '<=', $nip)
                        ->where('last', '>=', $nip)
                        ->count());
                    if ($banCount > 0) {
            ...
            $peerSums = NexusDB::table('peers')
                ->where('userid', (int) $user['id'])
                ->selectRaw('SUM(uploaded) AS pul, SUM(downloaded) AS pdl')
                ->first();
            $peerSums = (array) ($peerSums ?? []);

            $pul = $peerSums['pul'] ?? 0;
            $pdl = $peerSums['pdl'] ?? 0;

            $postsRow = NexusDB::select(
                'SELECT COUNT(DISTINCT p.id) AS c FROM posts AS p LEFT JOIN topics as t ON p.topicid = t.id
          	LEFT JOIN forums AS f ON t.forumid = f.id WHERE p.userid = '.(int) $user['id'].' AND f.minclassread <= '.(int) $CURUSER['class']
            );
