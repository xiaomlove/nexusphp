...
if (isset($_GET['action']) && $_GET['action'] == 'apply') {
    if (user_can('applylink')) {
        ...
        echo '<p align=left><b><font size=5>'.$lang_linksmanage['text_rules']."</font></b></p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.sprintf($lang_linksmanage['text_rule_one'], getSchemeAndHttpHost(), $SLOGAN, $siteName)."</p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.sprintf($lang_linksmanage['text_rule_two'], $siteName)."</p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.$lang_linksmanage['text_rule_three']."</p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.$lang_linksmanage['text_rule_four']."</p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.sprintf($lang_linksmanage['text_rule_five'], $siteName)."</p>\n";
        echo '<p align=left>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.sprintf($lang_linksmanage['text_rule_six'], $siteName)."</p>\n";

        echo '<p>'.$lang_linksmanage['text_red_star_required'].'</p>';
        ?>
...
elseif (! user_can('linkmanage')) {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'del') {
        ...
        NexusDB::table('links')->where('id', (int) $id)->delete();
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        ...
        $linkname = (string) $_POST['linkname'];
        $urlVal = (string) $_POST['url'];
        $titleVal = (string) $_POST['title'];
...
<?php
    ...
    $linkRows = NexusDB::table('links')->orderBy('id')->get();
    if (count($linkRows) > 0) {
        ...
        echo '<tr><td colspan=4>'.$lang_linksmanage['text_no_links_found'].'</td></tr>';
    }
    echo '</table>';
    ?>
<?php if (isset($_GET['action']) && $_GET['action'] == 'edit') {
        $id = intval($_GET['id'] ?? 0);
        $rowObj = NexusDB::table('links')->where('id', (int) $id)->first();
        if ($rowObj) {
            $row = (array) $rowObj;
            ?>
