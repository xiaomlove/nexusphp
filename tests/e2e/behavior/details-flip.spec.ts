 ...
 *   - `?hit=1`    — view-counter side effect, generated by index.php /
 *                   userhistory.php / myhr.php first-party links
...
test.describe('@behavior Strangler Fig flip: /details.php → /torrent/{id}', () => {
    ...
    test('/details.php?id=1&hit=1 stays on legacy (view-counter hatch)', async ({
        context,
        page,
    }) => {
        await loginAs(context, 'admin');
        ...
        // effect — the counter is not wired into TorrentDetail yet, so
        // these must fall through to the legacy page.
