...
- `?hit=1` — first-party "open from listing" link that increments
  the view counter (`torrents.views` / `torrents.hits`). The
  counter side effect is not wired into `TorrentDetail` yet, so
  these stay on legacy. Generated by `public/index.php`,
  `public/userhistory.php`, `public/myhr.php` and friends.
